...
RUN apt update && \
    apt install -y sudo bzip2 apache2 mariadb-server libapache2-mod-php7.0 php-apcu php-mysql php-dompdf php-zip php-xml php-xml-parser php-xml-serializer php-mbstring php-gd php-curl && \
	apt-get clean && \
	apt-get autoclean
...
RUN sed -i -e 's/;error_log = php_errors.log/error_log = \/var\/log\/php_errors.log/' /etc/php/7.0/apache2/php.ini  && \
	sed -i -e 's/^allow_url_fopen/;allow_url_fopen/' /etc/php/7.0/apache2/php.ini && \
	sed -i -e 's/;open_basedir =/open_basedir = \/var\/www\/html:\/data:\/tmp/' /etc/php/7.0/apache2/php.ini
